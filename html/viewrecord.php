    ...
    <main class="relative p-1.5 md:ml-64 h-auto pt-5">
      ...
      <section class="relative bg-white dark:bg-gray-900 p-2 sm:p-2 rounded-lg mb-2">
        ...
        <div
          ...
          <ul id="membershipList" class="max-w-full space-y-1 dark:text-white"
          ...
          <div class="grid grid-cols-2 gap-x-2 gap-y-4 p-1">
            ...
            <div
              ...
              <div class="relative overflow-x-auto mt-5">
                <table
                  class="w-full block text-sm text-left rtl:text-right text-gray-900 dark:text-white border dark:bg-gray-800 dark:border-gray-700 border-gray-200 rounded-lg">
            ...
            <div
              ...
              <div class="relative overflow-x-auto mt-5">
                <table
                  class="w-full block text-sm text-left rtl:text-right text-gray-900 border dark:text-white border-gray-200 rounded-lg">
        ...
        <div
          ...
          <ul id="medicalHistoryList"
            class="max-w-full space-y-1 text-gray-900 dark:text-white"
            style="font-size:14.6px;">
          </ul>
        </div>
        <!-- Dietary -->
  ...
  <script>
    ...
    async function loadMemberships(patientId) {
      try {
        ...
        const res = await apiFetch(`/DentalEMR_System/php/register_patient/patient_info.php?action=get_membership&patient_id=${patientId}&_t=${Date.now()}`);
    ...
    async function loadMedicalHistory(patientId) {
      try {
        ...
        const res = await apiFetch(`/DentalEMR_System/php/register_patient/patient_info.php?action=get_medical&patient_id=${patientId}&_t=${Date.now()}`);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
    ...
    async function loadDietaryHistory(patientId) {
      try {
        ...
        const res = await apiFetch(`/DentalEMR_System/php/register_patient/patient_info.php?action=get_dietary&patient_id=${patientId}&_t=${Date.now()}`);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
      ...
      if (membershipForm) {
        membershipForm.addEventListener("submit", async function(e) {
          ...
          try {
            ...
            const response = await apiFetch("/DentalEMR_System/php/register_patient/patient_info.php", {
              method: "POST",
              body: formData
            });
