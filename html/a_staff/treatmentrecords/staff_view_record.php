    ...
    if (!isset($_GET['uid'])) {
        echo "<script>
            if (!navigator.onLine) {
                ...
                window.location.href = '/DentalEMR_System/html/a_staff/treatmentrecords/view_info.php?offline=true&id=" . (isset($_GET['id']) ? $_GET['id'] : '') . "';
            } else {
...
<body class="bg-gray-50 dark:bg-gray-900">
    ...
    <script>
        ...
        if (oralHealthLink && patientId) {
            oralHealthLink.href = `staff_view_oral.php?uid=<?php echo $userId; ?>&id=${encodeURIComponent(patientId)}`;
        } else {
