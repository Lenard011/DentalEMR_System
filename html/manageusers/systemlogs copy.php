...
<body class="bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 antialiased">
    ...
    <nav class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 fixed top-0 left-0 right-0 z-40">
        <div class="px-4 py-3">
            <div class="flex items-center justify-between">
                <!-- Mobile Menu Button -->
                <button type="button" id="mobileMenuButton" class="lg:hidden p-2 rounded-lg text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700">
                ...
                <div class="flex items-center space-x-3">
                    <img src="https://th.bing.com/th/id/OIP.zjh8eiLAHY9ybXUCuYiqQwAAAA?r=0&rs=1&pid=ImgDetMain&cb=idpwebp1&o=7&rm=3"
                        class="h-8 w-8 object-contain" alt="MHO Dental Clinic Logo">
                    <span class="text-lg font-semibold truncate">MHO Dental Clinic</span>
                </div>

                <!-- User Profile -->
                <div class="flex items-center space-x-3">
                    <!-- Notifications (Optional) -->
                    <button type="button" class="p-2 relative text-gray-600 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-400">
    ...
    <div id="mobileSidebar" class="fixed inset-0 z-50 lg:hidden hidden">
        ...
        <div class="absolute inset-y-0 left-0 w-64 bg-white dark:bg-gray-800 shadow-xl">
            <div class="p-4 border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    <h2 class="text-lg font-semibold">Menu</h2>
                    <button onclick="toggleMobileSidebar()" class="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                    ...
                    <div id="pagination" class="px-4 py-3 border-t border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        ...
                        <nav class="flex items-center space-x-1">
                            <button onclick="systemLogs.changePage(systemLogs.currentPage - 1)" id="prevPage" disabled class="px-3 py-1.5 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed">
                                <i class="fas fa-chevron-left"></i>
                            </button>
                            <div id="pageNumbers" class="flex items-center space-x-1">
                                <!-- Page numbers will be inserted here -->
                            </div>
                            <button onclick="systemLogs.changePage(systemLogs.currentPage + 1)" id="nextPage" disabled class="px-3 py-1.5 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed">
    ...
    <script>
        ...
        class SystemLogsManager {
            ...
            setupEventListeners() {
                // Mobile menu
                document.getElementById('mobileMenuButton').addEventListener('click', () => this.toggleMobileSidebar());
                document.getElementById('userDropdownButton').addEventListener('click', (e) => {
                    e.stopPropagation();
                ...
                document.addEventListener('click', (e) => {
                    if (!e.target.closest('#userDropdown') && !e.target.closest('#userDropdownButton')) {
            ...
            renderLogs() {
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer transition-colors" 
                    onclick="systemLogs.showLogDetails(${JSON.stringify(log).replace(/"/g, '&quot;')})">
                    <td class="px-4 py-3" onclick="event.stopPropagation()">
                        <input type="checkbox" 
                               value="${log.id}"
                               onchange="systemLogs.toggleLogSelection(${log.id})"
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center space-x-2">
                            <button onclick="event.stopPropagation(); systemLogs.showLogDetails(${JSON.stringify(log).replace(/"/g, '&quot;')})" 
                                    class="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded"
                                    title="View details">
                                <i class="fas fa-eye text-blue-500"></i>
                            </button>
                            <button onclick="event.stopPropagation(); systemLogs.confirmDeleteLog(${log.id})" 
                                    class="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded"
                                    title="Delete log">
            ...
            `).join('');
                ...
                this.updateBulkActions(); // FIXED: Changed from updateSelectedCount() to updateBulkActions()
            }
            ...
            updatePagination() {
                ...
                if (this.currentPage > 1) {
                    pagesHTML += `<button onclick="systemLogs.changePage(1)" class="px-3 py-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700">1</button>`;
                    if (this.currentPage > 3) pagesHTML += `<span class="px-2">...</span>`;
                }
                ...
                for (let i = Math.max(1, this.currentPage - 1); i <= Math.min(this.totalPages, this.currentPage + 1); i++) {
                    if (i === this.currentPage) {
                        ...
                        pagesHTML += `<button onclick="systemLogs.changePage(${i})" class="px-3 py-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700">${i}</button>`;
                    }
                }

                // Show last page
                if (this.currentPage < this.totalPages) {
                    if (this.currentPage < this.totalPages - 2) pagesHTML += `<span class="px-2">...</span>`;
                    pagesHTML += `<button onclick="systemLogs.changePage(${this.totalPages})" class="px-3 py-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700">${this.totalPages}</button>`;
                }
            ...
            filterLogs(filter) {
                ...
                document.getElementById('filterAll').classList.remove('bg-blue-100', 'dark:bg-blue-900/30', 'text-blue-700', 'dark:text-blue-300');
                document.getElementById('filterActivity').classList.remove('bg-blue-100', 'dark:bg-blue-900/30', 'text-blue-700', 'dark:text-blue-300');
                document.getElementById('filterHistory').classList.remove('bg-blue-100', 'dark:bg-blue-900/30', 'text-blue-700', 'dark:text-blue-300');

                document.getElementById('filterAll').classList.add('bg-gray-100', 'dark:bg-gray-700', 'text-gray-700', 'dark:text-gray-300');
                document.getElementById('filterActivity').classList.add('bg-gray-100', 'dark:bg-gray-700', 'text-gray-700', 'dark:text-gray-300');
                document.getElementById('filterHistory').classList.add('bg-gray-100', 'dark:bg-gray-700', 'text-gray-700', 'dark:text-gray-300');

                document.getElementById(`filter${filter.charAt(0).toUpperCase() + filter.slice(1)}`).className =
                    'px-4 py-2.5 rounded-lg bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 font-medium';
            ...
            confirmDeleteLog(logId = null) {
                ...
                if (logId) {
                    document.getElementById('deleteModalTitle').textContent = 'Delete Log';
                    document.getElementById('deleteModalMessage').textContent =
                        'Are you sure you want to delete this log? This action cannot be undone.';
                } else {
                    document.getElementById('deleteModalTitle').textContent = 'Delete Selected Logs';
                    document.getElementById('deleteModalMessage').textContent =
                        `Are you sure you want to delete ${this.selectedLogs.size} selected logs? This action cannot be undone.`;
                }
            ...
            async loadStats() {
                try {
                    const response = await fetch(`/dentalemr_system/php/manageusers/get_stats.php?uid=<?php echo $userId; ?>`);
                    const data = await response.json();
                    ...
                    if (data.success) {
                        ...
                        document.getElementById('dbSize').textContent = `${(data.db_size / 1024 / 1024).toFixed(2)} MB`;
                    }
                } catch (error) {
            ...
            async loadQuickStats() {
                try {
                    const response = await fetch(`/dentalemr_system/php/manageusers/get_quick_stats.php?uid=<?php echo $userId; ?>`);
                    const data = await response.json();
            ...
            showLoadingState() {
                ...
                document.getElementById('logsBody').classList.add('hidden');
            }

            hideLoadingState() {
                document.getElementById('loadingState').classList.add('hidden');
                document.getElementById('logsBody').classList.remove('hidden');
            }
        ...
        const systemLogs = new SystemLogsManager();

        // Global functions for inline event handlers
        window.toggleMobileSidebar = () => systemLogs.toggleMobileSidebar();
        window.refreshLogs = () => systemLogs.refreshLogs();
        window.debouncedSearch = () => systemLogs.debouncedSearch();
        window.clearSearch = () => systemLogs.clearSearch();
        window.filterLogs = (filter) => systemLogs.filterLogs(filter);
        window.toggleAdvancedFilters = () => systemLogs.toggleAdvancedFilters();
        window.applyAdvancedFilters = () => systemLogs.applyAdvancedFilters();
        window.clearFilters = () => systemLogs.clearFilters();
        window.sortTable = (field) => systemLogs.sortTable(field);
        window.changePage = (page) => systemLogs.changePage(page);
        window.changeLimit = (limit) => systemLogs.changeLimit(limit);
        window.toggleSelectAll = (checkbox) => systemLogs.toggleSelectAll(checkbox);
        window.toggleLogSelection = (logId) => systemLogs.toggleLogSelection(logId);
        window.clearSelection = () => systemLogs.clearSelection();
        window.showLogDetails = (log) => systemLogs.showLogDetails(log);
        window.closeDetailsModal = () => systemLogs.closeDetailsModal();
        window.confirmDeleteLog = (logId) => systemLogs.confirmDeleteLog(logId);
        window.confirmBulkDelete = () => systemLogs.confirmDeleteLog();
        window.confirmDelete = () => systemLogs.confirmDelete();
        window.closeDeleteModal = () => systemLogs.closeDeleteModal();
        window.showExportModal = () => systemLogs.showExportModal();
        window.closeExportModal = () => systemLogs.closeExportModal();
        window.proceedExport = () => systemLogs.proceedExport();
        window.exportSelected = () => systemLogs.exportSelected();

        // Also fix the changePage function references in pagination
        window.changePage = (page) => systemLogs.changePage(page);
    </script>
