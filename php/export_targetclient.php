...
// FIX: Don't cast to int, keep as string to accept 'both'
$part = isset($_GET['part']) ? $_GET['part'] : '1'; // '1', '2', or 'both'
$exportType = isset($_GET['type']) ? $_GET['type'] : 'excel'; // excel or csv or html

// Validate part parameter
if (!in_array($part, ['1', '2', 'both'])) {
    $part = '1';
}
...
// Debug: Log what part we're processing
error_log("Export requested for part: $part, year: $year, type: $exportType");

// Export based on part
if ($part === 'both') {
    error_log("Calling exportBothParts()");
    exportBothParts($conn, $year, $exportType);
} elseif ($part === '2') {
    error_log("Calling exportPart2()");
    exportPart2($conn, $year, $exportType);
} else {
    error_log("Calling exportPart1() - default");
    exportPart1($conn, $year, $exportType);
}

// NEW FUNCTION: Export both parts combined
function exportBothParts($conn, $year, $exportType)
{
    error_log("Inside exportBothParts function");
    
    // Fetch patients for the year with all data
    $sql = "
    SELECT 
        p.*,
        -- Part 1 data
        ...
        -- Part 2 data
        t.oe, t.iiohc, t.aebf, t.tfa, t.stb, t.ohe, t.ecc, t.art, t.ops, 
        t.pfs, t.tf, t.pf, t.gt, t.rp, t.rut, t.ref, t.tpec, t.dr,
        t.bohc_0_11, t.bohc_1_4, t.bohc_5_9, t.bohc_10_19, t.bohc_20_59, 
        t.bohc_60_plus, t.bohc_pregnant, t.remarks
    FROM patients p
    ...
    if ($exportType == 'excel') {
        error_log("Exporting to Excel format");
        exportBothPartsToExcel($result, $year);
    } elseif ($exportType == 'csv') {
        error_log("Exporting to CSV format");
        exportBothPartsToCSV($result, $year);
    } else {
        error_log("Invalid export type: $exportType");
        die("Invalid export type.");
    }
}

// FIXED: Added proper column count for the complete report table
function exportBothPartsToExcel($result, $year)
{
    // Total columns = 4 (basic info) + 2 (sex) + 1 (address) + 1 (dob) + 10 (age groups) + 1 (indigenous) + 2 (oral health) + 3 (DMFT) + 18 (services) + 7 (BOHC) + 1 (remarks) = 50 columns
    $totalColumns = 50;
    
    header('Content-Type: application/vnd.ms-excel');
    header('Content-Disposition: attachment;filename="Target_Client_List_Complete_' . $year . '_' . date('Ymd_His') . '.xls"');
    ...
    echo "th, td { border: 1px solid #000; padding: 3px; text-align: center; font-size: 9px; }";
    echo "th { background-color: #4F81BD; color: white; font-weight: bold; }";
    echo ".header-bg { background-color: #D9E1F2; }";
    echo ".section-title { background-color: #95B3D7; color: #000; font-weight: bold; text-align: center; font-size: 10px; }";
    echo ".part1-bg { background-color: #EAF1DD; }";
    echo ".part2-bg { background-color: #FDE9D9; }";
    echo ".patient-name { font-size: 8px; line-height: 1.1; }";
    echo "</style>";
    echo "</head>";
    echo "<body>";

    echo "<h2 style='text-align:center;font-size:14px;'>TARGET CLIENT LIST FOR ORAL HEALTH CARE AND SERVICES</h2>";
    echo "<h3 style='text-align:center;font-size:12px;'>Year: " . $year . " - Complete Report (Parts 1 & 2 Combined)</h3>";
    echo "<p style='text-align:center;font-size:9px;'>Generated on: " . date('F d, Y h:i A') . "</p>";

    echo "<table>";

    // PART 1 HEADERS - This spans the first 26 columns (1-26)
    echo "<tr class='section-title'>";
    echo "<td colspan='26'><strong>PART 1: BASIC INFORMATION AND ASSESSMENT</strong></td>";
    // Remaining columns for Part 2
    echo "<td colspan='24'><strong>PART 2: SERVICES PROVIDED AND BASIC ORAL HEALTH CARE</strong></td>";
    echo "</tr>";
    ...
    echo "<th colspan='2'>Oral Health<br>Status for<br>Children</th>";
    echo "<th colspan='3'>DMFT</th>";
    // Part 2 columns
    echo "<th colspan='18'>Oral Health Services Provided</th>";
    echo "<th colspan='7'>Provided with Basic Oral Health Care (BOHC)</th>";
    echo "<th rowspan='3'>Remarks</th>";
    echo "</tr>";

    // Second header row
    echo "<tr class='header-bg'>";
    // Part 1 - Sex subheaders
    ...
    // Part 1 - Oral health
    echo "<th>Orally Fit<br>Upon<br>Exam</th>";
    echo "<th>Orally Fit<br>After<br>Rehab</th>";
    // Part 1 - DMFT
    echo "<th>Decayed<br>Tooth</th>";
    echo "<th>Missing<br>Tooth</th>";
    echo "<th>Filled<br>Tooth</th>";
    // Part 2 - Placeholders for 18 service columns
    for ($i = 0; $i < 18; $i++) echo "<th></th>";
    // Part 2 - BOHC age groups
    echo "<th>0-11<br>mos.</th>";
    echo "<th>1-4<br>y/o</th>";
    echo "<th>5-9<br>y/o</th>";
    echo "<th>10-19<br>y/o</th>";
    echo "<th>20-59<br>y/o</th>";
    echo "<th>>60<br>y/o</th>";
    echo "<th>Pregnant</th>";
    echo "</tr>";
    ...
    // Third header row
    echo "<tr class='header-bg'>";
    // Part 1 - Empty cells for alignment
    echo "<th></th><th></th>"; // Sex
    echo "<th></th><th></th><th></th><th></th><th></th><th></th><th></th>"; // Age groups 0-11 to >=60
    echo "<th>10-14<br>y/o</th>"; // Pregnant subcolumns
    echo "<th>15-19<br>y/o</th>";
    echo "<th>20-49<br>y/o</th>";
    echo "<th></th><th></th>"; // Oral health
    echo "<th></th><th></th><th></th>"; // DMFT
    // Part 2 - Service codes
    echo "<th>OE</th><th>IIOHC</th><th>AEBF</th><th>TFA</th><th>STB</th><th>OHE</th><th>E&CC</th><th>ART</th>";
    echo "<th>OPS</th><th>PFS</th><th>TF</th><th>PF</th><th>GT</th><th>RP</th><th>RUT</th><th>Ref</th><th>TPEC</th><th>Dr</th>";
    // Part 2 - BOHC placeholders
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td style='text-align:left;' class='patient-name'>" . htmlspecialchars($row['surname'] . ', ' . $row['firstname'] . ' ' . $row['middlename']) . "</td>";
        ...
        // Address and DOB
        echo "<td style='text-align:left;font-size:8px;'>" . htmlspecialchars($row['address']) . "</td>";
        ...
        // PART 2 DATA
        // Oral Health Services (18 columns)
        ...
        // Remarks
        echo "<td style='text-align:left;font-size:8px;'>" . (!empty($row['remarks']) ? htmlspecialchars($row['remarks']) : '') . "</td>";
    ...
    echo "<p>Report includes both Part 1 (Basic Information) and Part 2 (Services Provided) in one file</p>";
...
function exportBothPartsToCSV($result, $year)
{
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment;filename="Target_Client_List_Complete_' . $year . '_' . date('Ymd_His') . '.csv"');
    header('Cache-Control: max-age=0');

    $output = fopen('php://output', 'w');

    // Headers for complete report - All 50 columns
    $headers = [
        // Part 1 Headers (26 columns)
        'No.',
