...
try {
    ...
    if ($isOfflineMode) {
        // ================= OFFLINE MODE =================
        // Get offline patients from localStorage via JavaScript
        // This will be handled client-side, so we return empty data structure
        $patients = [];
        $total = 0;
        $addresses = [];
    } else {
    ...
    echo json_encode(["error" => $e->getMessage(), "isOffline" => $isOfflineMode]);
}
