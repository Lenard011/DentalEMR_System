...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (isset($_GET['action']) && $_GET['action'] === 'get_patient') {
        ...
        if (!$id) exit(json_encode(["error" => "No patient ID provided."]));
        ...
        if (!$res) exit(json_encode(["error" => "Patient not found."]));

        echo json_encode(["success" => true, "patient" => $res]);
        exit;
    }
    ...
    // GET MEMBERSHIP INFO
    // ========================
    if (isset($_GET['action']) && $_GET['action'] === "get_membership") {
        ...
        $membership = [];
        if ($row) {
            if ($row['nhts_pr']) $membership[] = ["field" => "nhts_pr", "label" => "National Household Targeting System - Poverty Reduction (NHTS)"];
            if ($row['four_ps']) $membership[] = ["field" => "four_ps", "label" => "Pantawid Pamilyang Pilipino Program (4Ps)"];
            if ($row['indigenous_people']) $membership[] = ["field" => "indigenous_people", "label" => "Indigenous People (IP)"];
            if ($row['pwd']) $membership[] = ["field" => "pwd", "label" => "Person with Disabilities (PWD)"];
            if ($row['philhealth_flag']) $membership[] = ["field" => "philhealth_flag", "label" => "PhilHealth: (" . ($row['philhealth_number'] ?: "N/A") . ")"];
            if ($row['sss_flag']) $membership[] = ["field" => "sss_flag", "label" => "SSS: (" . ($row['sss_number'] ?: "N/A") . ")"];
            if ($row['gsis_flag']) $membership[] = ["field" => "gsis_flag", "label" => "GSIS: (" . ($row['gsis_number'] ?: "N/A") . ")"];
        }

        echo json_encode(["success" => true, "membership" => $membership, "values" => $row ?? []]);
        exit;
    }
    ...
    // GET VITAL SIGNS
    // ========================
    if (isset($_GET['action']) && $_GET['action'] === 'get_vitals' && isset($_GET['patient_id'])) {
        $pid = intval($_GET['patient_id']);
        $stmt = $conn->prepare("SELECT blood_pressure, pulse_rate, temperature, weight, recorded_at 
                                FROM vital_signs 
                                WHERE patient_id=? 
        ...
        echo json_encode(['success' => true, 'vitals' => $result]);
        exit;
    }
    ...
    // GET MEDICAL HISTORY
    // ========================
    if (isset($_GET['action']) && $_GET['action'] === "get_medical") {
        $patient_id = intval($_GET['patient_id'] ?? 0);
    ...
    // GET DIETARY HABITS
    // ========================
    if (isset($_GET['action']) && $_GET['action'] === "get_dietary") {
        $patient_id = intval($_GET['patient_id'] ?? 0);
    ...
    // ========================
    // LEGACY: get_vital_signs
    // ========================
    if (isset($_GET['vital_signs']) && isset($_GET['patient_id'])) {
        $pid = intval($_GET['patient_id']);
        $stmt = $conn->prepare("SELECT * FROM vital_signs WHERE patient_id=? ORDER BY recorded_at DESC");
        $stmt->bind_param("i", $pid);
        $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$id) exit(json_encode(["error" => "Missing patient ID."]));
    $action = $_POST['action'] ?? '';
    ...
    if ($action === 'save_vitals') {
        $stmt = $conn->prepare("INSERT INTO vital_signs 
            (patient_id, blood_pressure, pulse_rate, temperature, weight, recorded_at)
            VALUES (?, ?, ?, ?, ?, NOW())");
        $stmt->bind_param("issss", $id, $_POST['blood_pressure'], $_POST['pulse_rate'], $_POST['temperature'], $_POST['weight']);
    ...
    $stmt = $conn->prepare("UPDATE patients SET 
        firstname=?,surname=?,middlename=?,date_of_birth=?,sex=?,age=?,place_of_birth=?,address=?,occupation=?,guardian=? 
        WHERE patient_id=?");
    $stmt->bind_param(
        "ssssssssssi",
        $_POST['firstname'],
