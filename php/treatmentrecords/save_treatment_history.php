...
try {
    ...
    $tableCheck = $db->query("SHOW TABLES LIKE 'treatment_history'")->rowCount() > 0;
    
    if (!$tableCheck) {
        throw new Exception("Treatment history table not found. Please create it first.");
    }
    ...
    foreach ($treatments as $index => $t) {
        ...
        // **CHECK: If treatment already exists for same patient+tooth+date, UPDATE it**
        // Otherwise, INSERT new record
        $checkSql = "SELECT history_id FROM treatment_history 
                     WHERE patient_id = ? 
                     AND tooth_id = ? 
                     AND treatment_date = ?";
        ...
        if ($existing) {
            ...
            $updateSql = "UPDATE treatment_history 
                          SET treatment_code = ?, created_at = ?
                          WHERE history_id = ?";
            $updateStmt = $db->prepare($updateSql);
            $result = $updateStmt->execute([$treatment_code, $date_time, $existing['history_id']]);
            
            if ($result) {
                $results['updated']++;
                error_log("UPDATED treatment for patient $patient_id, tooth $fdi on $date");
            } else {
                $results['skipped']++;
            ...
            $insertSql = "INSERT INTO treatment_history 
                          (patient_id, tooth_id, treatment_code, treatment_date, created_at) 
                          VALUES (?, ?, ?, ?, ?)";
            $insertStmt = $db->prepare($insertSql);
            $result = $insertStmt->execute([$patient_id, $tooth_id, $treatment_code, $date, $date_time]);
            
            if ($result) {
                $results['inserted']++;
                $newId = $db->lastInsertId();
                error_log("INSERTED new treatment ID $newId for patient $patient_id, tooth $fdi on $date");
            } else {
                $results['skipped']++;
